...
use crate::{ConfluxSimulationError, execution::PreparedTransactionExecution};
...
) -> Result<ChangeMetadata, ConfluxSimulationError> {
...
impl MetadataReader<'_> {
    fn read_erc20(&mut self, contract: Address) -> Result<Erc20Metadata, ConfluxSimulationError> {
        Ok(Erc20Metadata {
    ...
    fn read_erc721(
        &mut self,
        collection: Address,
    ) -> Result<Erc721CollectionMetadata, ConfluxSimulationError> {
    ...
    ) -> Result<Option<T>, ConfluxSimulationError> {
