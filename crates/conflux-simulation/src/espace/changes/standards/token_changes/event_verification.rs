...
use crate::espace::{
    EspaceChangesError, EspaceExecutedTransaction, EspaceExecutionPosition, EspaceFrameId,
    EspaceStateReader,
};
...
pub(super) struct TokenEventVerification<'a> {
    ...
    pub(super) execution: &'a EspaceExecutedTransaction,
    pub(super) frame_id: EspaceFrameId,
    pub(super) previous: &'a EspaceStateReader,
    pub(super) current: &'a EspaceStateReader,
...
impl TokenEventVerification<'_> {
    ...
    fn verify_erc721_transfer(
        ...
        let before = read_erc721_owner(self.previous, contract, token_id)?;
        let after = read_erc721_owner(self.current, contract, token_id)?;
        ...
        let approval_before = if before.is_some() {
            read_erc721_approval(self.previous, contract, token_id)?
        } else {
            read_erc721_approval_optional(self.previous, contract, token_id)?
        };
        ...
        let approval_after = if after.is_some() {
            read_erc721_approval(self.current, contract, token_id)?
        } else {
            read_erc721_approval_optional(self.current, contract, token_id)?
        };
        ...
        self.final_state_expectations.insert(
            FinalStateQuery::Owner { contract, token_id },
            ExpectedFinalValue::Owner(after),
        );
        self.final_state_expectations.insert(
