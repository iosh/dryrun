...
use crate::{
    ConfluxSimulationError, PreparedCoreSpaceSimulation, PreparedEspaceSimulation,
...
impl ConfluxSimulation {
    ...
    pub(crate) async fn prepare_espace_completed_transaction(
        &self,
    ...
    pub async fn prepare_espace_transaction(
        ...
        let context = self.load_espace_context(block).await?;
        let transaction =
            complete_espace_transaction(self.provider.as_ref(), &context, request).await?;
        self.prepare_espace_completed_transaction(context, transaction)
            .await
    }
    ...
    pub(crate) async fn prepare_core_space_completed_transaction(
        &self,
    ...
    pub async fn prepare_core_space_transaction(
        ...
        .await?;
        self.prepare_core_space_completed_transaction(context, transaction)
            .await
    }
