...
pub(super) fn collect_cross_space_call(
    ...
    machine: &Machine,
    spec: &Spec,
) -> Result<Option<(CrossSpaceTransferOperation, usize)>, ConfluxEngineError> {
...
fn collect_transfer_to_espace(
    ...
    match (transfer.from, transfer.to) {
        ...
        _ => Err(ConfluxEngineError::analysis_failed(
...
fn required_transfer<'a>(
    ...
    observation_index: usize,
    operation: &str,
) -> Result<InternalTransferFacts<'a>, ConfluxEngineError> {
