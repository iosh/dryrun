...
fn raw_vote_amount(vote_count: u64) -> Result<U256, ConfluxEngineError> {
    U256::from(vote_count)
        .checked_mul(u256_from_cfx(*cfx_parameters::staking::POS_VOTE_PRICE))
        .ok_or_else(|| ConfluxEngineError::analysis_failed("Core Space PoS vote value overflowed"))
}
...
fn verify_after_staking_coverage(after_state: &PoSStateValues) -> Result<(), ConfluxEngineError> {
    for (account, pos_identifier) in &after_state.sender_pos_identifiers {
        if pos_identifier.is_zero() {
            continue;
        }
