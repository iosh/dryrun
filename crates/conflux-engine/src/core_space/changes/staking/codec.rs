...
use crate::{ConfluxEngineError, primitive::b256_from_cfx};
...
pub(super) fn decode_vote_lock_call(
    input_len: usize,
    input_prefix: &[u8],
) -> Result<Option<VoteLockCall>, ConfluxEngineError> {
...
pub(super) fn decode_pos_call(
    input_len: usize,
    input_prefix: &[u8],
) -> Result<Option<PoSCall>, ConfluxEngineError> {
...
) -> Result<Vec<PoSEvent>, ConfluxEngineError> {
...
fn read_call_word(
    ...
    offset: usize,
    field: &str,
) -> Result<[u8; 32], ConfluxEngineError> {
    let end = offset + 32;
...
fn decode_pos_event(log: &LogEntry) -> Result<PoSEvent, ConfluxEngineError> {
    if log.topics.len() != 2 {
    ...
    if event_signature == Register::SIGNATURE_HASH {
        ...
        Err(ConfluxEngineError::analysis_failed(
